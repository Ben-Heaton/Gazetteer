<?php
    ...
    // Insert the variables into the API URL address.
    $url = "https://api.openweathermap.org/data/2.5/forecast?lat=$latSafe&lon=$lngSafe&appid=" . WEATHERFORECAST_KEY . "&units=metric";
    ...
    if ($cURL_ERROR_NUMBER !== 0) {
        ...
        foreach ($decoded['list'] as $days) {
            if (str_contains($days['dt_txt'], '12:00:00')) {
                array_push($infoArray, [
                    "main" => $days['weather'][0]['main'],
